...
## v2 — V2-14 (Dokumentation)
- `DEMO.md` ist auf fünf Akte plus Vorspann neu geschrieben, mit Zeitmarken bis 4:15. Akt 4 bekommt mit 70 Sekunden den größten Block — er ist der Grund für diese Überarbeitung. Die Zahlen in der Ortstabelle sind die tatsächlich gerechneten, nicht gerundete Wunschwerte.
- Zu `DEMO.md` steht ausdrücklich dabei, dass Akt 2 nicht kommentiert wird („Nicht kommentieren", „Nichts hinzufügen, nichts vorwegnehmen"). Der Schutz aus §6 gilt auch für die Person, die vorträgt.
- `HANDOVER.md` ist **additiv** ergänzt: alles oberhalb der Trennlinie bleibt, einschließlich des vollständigen Inbetriebnahme-Abschnitts. Der Nachtrag nennt die neue Aktstruktur, die neuen Dateien, die drei Entscheidungen mit dem meisten Widerspruchspotenzial und den Hinweis auf die Gatter-Lücke bei kaputten Testdateien.
- Jede Beschriftung, die `DEMO.md` zum Anklicken nennt, wurde gegen `copy.de.js` geprüft.
